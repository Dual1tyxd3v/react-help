...
App.vue<pre>
...
const {collection, addColl} = useMy();</pre><hr>
Чтобы использовать хуки в Options API нужно вернуть их в функции setup чтобы они были доступны на объекте this<pre>
export default {
  setup() {
    const { collection } = useMy();
    return { collection };
  }
  mounted() {
...
Чтобы хук срабатывал каждый раз при изменении переданного аргумента необходимо внутри него использовать watch/watchEffect<pre>
const useMy = (arg) => {
  const data = ref(null);
  ...
  [
    `Кастомные директивы`,
    `Vue позволяет создавать кастомные директивы если необходимо взаимодействовать с DOM напрямую. По сути это объект содержащий методы жизненного цикла. Например директива позволяющая делать фокусировку на элементе при монтировании компонента<pre>
    export const vFocus = {
      mounted(el) {
        el.focus();
      }
    ...
    &lt;template&gt;
      &lt;input v-focus /&gt;
    &lt;/template&gt;</pre>
    Называться такие директивы должны с приставки v. По умолчанию каждый метод жизненного цикла получает в виде 1 аргумента сам элемент на котором была указана текущая директива. Далее их можно использовать в разметке с помощью cebab-case, vue автоматически определит нужную директиву<br>
    Доступные хуки - created, beforeMount, mounted, beforeUpdate, updated, beforeUnmount, unMounted<br>
    Хуки получают следующие аргументы:<br>
    1. el - сам элемент<br>
    2. binding - объект содержащий следующие свойства<br>
    -2.1 value - значение переданное в директиву(v-focus="some")<br>
    -2.2 oldValue - предыдущее значение. Данное свойство доступно только в хуках beforeUpdated, updated<br>
    -2.3 arg - аргументы директивы (v-focus:arg)<br>
    -2.4 modifiers - объект с модификаторами(v-focus.some.x => { some: true, x: true})<br>
    -2.5 instance - сущность компонента в котором используется директива<br>
    -2.6 dir - объект директивы<br>
    3. vnode<br>
    4. prevNode<hr>
    Кастомную директиву также можно зарегистрировать глобально<pre>
    const app = createApp(App);
    app.directive('focus', {...});</pre>
    В названии необязательно указывать преффикс v, vue автоматически это сделает сам. Во 2 аргумент передается объект с директивой<hr>
    Если в директиве планируется использовать только хуки - mounted и updated то можно описать ее в виде функции<pre>
    export const vFocus = (el) => el.focus();</pre><hr>
    В случае использования такой директивы на компоненте, она будет использована на root элементе этого компонента. Если такого нет то получим ошибку`
  ],
  [
    `Роутинг`,
...
Без указания mode анимация компонентов начнется одновременно<hr>
Чтобы применить анимацию к элементу который не меняется можно задать ему динамический ключ<pre>
const t = ref(0);
setInterval(() => t.value++, 1000);
...
&lt;template&gt;
  &lt;transition ... &gt;
    &lt;p :key="t"&gt;Some static text&lt;/p&gt;
  &lt;/transition&gt;
&lt;/template&gt;</pre>`,
  ],
  [
...
this.$store.имя_store.нужное_свойство_метод (this.$store.app.volume)`,
  ...
  [
    `Плагины`,
    `Плагины позволяют добавить дополнительную функциональность на верхний уровень приложения а также уменьшить количество импортов. Полезно при:<br>
    1. нужно зарегистрировать глобально 1 и более компонентов/директив<br>
    2. сделать ресурс доступным для внедрения в app.provide<br>
    3. добавление глобальных свойств или методов<br>
    4. использование сторониих библиотек с использованием вариантов выше<br>
    По сути плагин это отдельный файл в виде объекта с методом install или функция. 1 аргумент объект app, 2 аргумент объект переданных настроек
    При таком подключении библиотека становится доступна на объекте this<br>
    axiosPlug.js<pre>
...
Далее в компоненте просто обращаемся к ранее указанному свойству на объекте this<br>
Пример с объектом настроек<pre>
<b>plugin.js</b>
export const myPlugin = {
  install(app, options) {
    app.config.globalProperties.$translate = (arg) => {
      return arg.split('.').reduce((acc, v) => {
        if (acc) return acc[v]
      }, options)
    }
  }
...
<b>main.js</b>
const app = createApp(App);
app.use(myPlugin, { greetings: { hello: 'Hi' } });
<b>App.vue</b>
...
&lt;template&gt;
