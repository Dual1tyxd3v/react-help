...
export const Css = [
  [`text-align`, `Выравнивает содержимое внутри контейнера по горизонтали(устаревший метод)`],
  [
    `filter`,
    `Свойство позволяет применить к элементу различные графические эффекты (blur, brightness, drop-shadow...)<br>
    <b>drop-shadow(x, y, blur, color)</b> - указывает что элемент отбрасывает тень где x и y сдвин по осям, blur - размытие и color цвет тени`,
  ],
  [
    `object-fit`,
  ...
  [
    `appearance`,
    `Позволяет задать внешний вид элементам основываясь на настройках системы. Самое используемое значение - none, все остальные практически не работают<br>
    <b>none</b> - полностью сбрасывает стилистику элемента`,
  ],
  [
    `aspect-ratio`,
    `Свойство позволяет задать элементу соотношение ширины к высоте. Принимает числовые значения(например 2 - ширина в 2 раза больше высоты). Даже если в будущем размеры элемента изменятся то соотношение сохранится`,
  ],
  [
    `backdrop-filter`,
    `Свойство позволяет задать различные фильтры относительно фона под элементом`,
  ],
  [
    `accent-color`,
  ...
  [
    `mask`,
    `Позволяет создать шаблон из изображения по которому затем будет создан фон заданного цвета. Т.к. свойство достаточно новое для многих браузеров требуется префикс. Работает только с png, svg и градиентами(по сути накладывает на элемент заданное изображение/градиент и отображает ту часть элемента что под непрозрачной частью, например на img установить линейный градиент чтобы создать эффект исчезновения картинки). Таким образом можно скрыть часть элемента как clip-path только более изящно<br>
...
Общий селектор * имеет 0 спецификацию`,
  ...
  [
    `Единицы изменения`,
    `em - единица измерения на основе font-size родителя. (1em = font-size). Подходит для позиционирования.
    <hr>
    rem - единица равна величине font-size тэга html
    <hr>
  ...
  [`Семантические теги`, `предназначены для поисковиков и скринридеров`],
  [
    `БЭМ`,
  ...
  [`Шрифты`, `с засечками - serif, без засечек sans-serif`],
  [
...
2. Установить родителю свойство overflow: hidden`,
  ...
  [`outline-offset`, `Свойство устанавливает отступ между outline и border`],
  [`:nth-child(2n)`, `Псевдокласс позволяет задать свойста для каждого 2 элемента`],
  [
  ...
  [
    ...
    radial-gradient(x, y) - круговой градиент где х начальный цвет от центра и у конечный цвет у края круга<br>
    conic-gradient(from x, c1 a1, c2 a2...) - позволяет задать конический градиент. Градиент идет от центра(по default в центре элемента) до точки х(угол в deg)(0 вверху 180 внизу по часовой стрелке). Переход цвета идет от указаного радиуса по часовой стрелке. c1 - цвет в начальной точке, a1 - на сколько градусов идет сектор с этим цветом. c2 - следующий цвет который должен начаться в конце предыдущего сектора и займет a2 градусов. Центр также можно сместить указав at x y<br>
    conic-gradient(from 0deg at 0 0, blue 90deg, green 360deg)`,
  ],
  [
    `list-style-type`,
  ...
  [
    `@property`,
    `Позволяет создавать переменные в CSS чьи значения можно менять в стилях разных элементов<pre>
@property --x {    <sup>1</sup>
  syntax: '&lt;color&gt;';      <sup>2</sup>
  inherits: true;       <sup>3</sup>
  initial-value: red;       <sup>4</sup>
}
p {
  color: var(--x);      <sup>5</sup>
}
a {
  --x: green;       <sup>6</sup>
  color: var(--x);
...
.someText {
  --x: black;
  transition: --x 0.5s;     <sup>7</sup>
}
.someText:hover {
  --x: white;
}</pre>
1. сначала создаем переменную используя общепринятый синтаксис с --<br>
2. далее необходимо указать тип данных в '&lt;...&gt;'<br>
3. затем указываем что свойство наследуется<br>
4. задаем начальное значение<br>
5. далее можем использовать переменную как обычную с помощью var(var_name)<br>
6. также мы можем просто переопределить ее значение указав новое. В случае если значение не корректное будет применено начальное<br>
7. также мы можем использовать переменную в transition для анимации`,
  ],
  [
    `hyphens`,
