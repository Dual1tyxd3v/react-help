...
const a = .3 - .2;
const b = .2 - .1;
console.log(a === b);</pre>Результатом этого сравнения будет false т.к. такие числа имеют не точности и для подобного сравнения сначала нужно преобразовать их в целые числа<br>
Такое происходит потому что числа в JS хранятся в памяти в двоичной системе, а поскольку дробное число не может быть полностью записано в этой системе(бесконечная двоичная дробная часть) то при сложении таких чисел оно сокращается что приводит к неточности`
  ],
  [
    `.toString(x?)`,
  ...
  B.prototype = Object.create(A.prototype);   <sup>2</sup>
  B.prototype.constructor = B;</pre>
  ...
  2) затем с помощью Object.create наследуем прототип первого объекта<br>
  Т.к. после такого наследования мы перезаписываем прототип B, то необходимо заного указать ему конструктор<pre>
  class A {
    constructor(a, b) {
      this.a = a;
      this.b = b;
...
<b>obj1.someMethod.call(obj2);</b>`
  ...
  [
    ...
    1) у стрелочных ф-ций нет своего контекста. Он определяется в момент создания функции и привязывается к ближайшему окружению<pre>
const test = {
  a: 4,
  b: () => console.log(this.a)
}
test.b();   // undefined</pre>
Несмотря на то что функция объявлена внутри объекта, создается она все же во время присваивания значения и поэтому объекта еще не существует и this ссылается на глобальную область. Это называется раннее связывание<br>
    2) если декларативная ф-ция была вызвана просто как функция то ее объект this будет ссылаться на глобальную область видимости (window) в нестрогом режиме или undefined в строгом<br>
...
function quick(arr) {
  ...
  [
    ...
    <b>el.getComputedStyle().x</b> - позволяет получить любое CSS свойство элемента НО уже вычисленное на этапе отрисовки.<br>
    <b>el.getPropertyValue('--some-var')</b> - позволяет получить значение CSS переменной на каком либо элементе<br>
    <b>el.setProperty('--var', 'red')</b> - позволяет задать переменную/свойство для элемента`
  ],
  [
    `window.page?Offset`,
